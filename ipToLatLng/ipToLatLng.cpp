...
/*
// using the json object we obtained, get Lat, Lng from it
void ipToLatLng::get_LatLng( json::value json ) {
    if (json.is_null())
	return;	// json object is empty

    if ( json.at(U("geoplugin_latitude")).is_double() ) {
	ucout << U("Parsing my latitude...") << endl;
	
	mylat = json.at(U("geoplugin_latitude")).as_double();
    }
    if ( json.at(U("geoplugin_longitude")).is_double() ) {
	ucout << U("Parsing my longitude...") << endl;

	mylng = json.at(U("geoplugin_longitude")).as_double();
    }
